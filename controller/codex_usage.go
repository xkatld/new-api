...
func fetchCodexChannelWhamData(
	...
	client, err := service.GetHttpClientWithProxy(ch.GetProxyForAttempt())
	if err != nil {
		common.ApiError(c, err)
		return
	}
	...
	if (statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden) && strings.TrimSpace(oauthKey.RefreshToken) != "" {
		...
		res, refreshErr := service.RefreshCodexOAuthTokenWithProxy(refreshCtx, oauthKey.RefreshToken, ch.GetProxyForAttempt())
		if refreshErr == nil {
